...
students <- read.csv("input/H_all.csv", header = TRUE)
################################################
...
potGrads_Sem2 <- students %>% group_by(student_id) %>% filter(year == 2017, Total_Achieved >= 250, Study_Period != "Semester 1") %>%  select(student_id, surname_name, first_name,  Total_Enrolled, Total_Achieved, study_package)
################################################
potGrads_Sem1 <- unique(potGrads_Sem1)
potGrads_Sem1 <- potGrads[order(potGrads_Sem1$surname_name),]
################################################
potGrads_Sem2 <- unique(potGrads_Sem2)
potGrads_Sem2 <- potGrads[order(potGrads_Sem2$surname_name),]
################################################
write.csv(potGrads_Sem1, file = "potGrads_Sem1.csv")
################################################
write.csv(potGrads_Sem2, file = "potGrads_Sem2.csv")
################################################
################################################
################################################