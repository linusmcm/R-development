...
students <- read.csv("input/M3_R_Q_G.csv", header = TRUE)
################################################
students <- renameDF(students)
################################################s
potGrads_Sem1 <- students %>% group_by(student_id) %>% filter(year == 2017, Total_Achieved >= 300, Study_Period != "Semester 2" ) %>%  select(student_id, surname_name, first_name,  Total_Enrolled, Total_Achieved, study_package)
################################################
potGrads_Sem1 <- unique(potGrads_Sem1)
potGrads_Sem1 <- potGrads_Sem1[order(potGrads_Sem1$surname_name),]
################################################
################################################
write.csv(potGrads_Sem1, file = "PotC_M3_R_Q_G.csv")
################################################
